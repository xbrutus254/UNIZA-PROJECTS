...
namespace structures
{
	...
	Structure& Array<T>::operator=(const Structure& other)
	{
		if (this != &other)
		{
			*this = dynamic_cast<const Array<T>&>(other); // pretypovanie to èo je () na to èo je <>
		}
		return *this;
	}
	...
	const T Array<T>::operator[](const int index) const
	{
		...
		byte* bPrtVoVectore = vector_->getBytePointer(indexVoVectore);
		T* TPtrVoVectore = reinterpret_cast<T*>(bPrtVoVectore);
