...
namespace structures 
{
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (item != nullptr) {
			...
			throw std::out_of_range("SequenceTable<K, T>::operator[].");
		}
	}
	...
	inline void SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		...
		if (!containsKey(key)) {
			...
			throw std::exception("SequenceTable<K, T>::insert.");
		}
	}
	...
	inline T SequenceTable<K, T>::remove(const K & key)
	{
		...
		if (item != nullptr) {
			...
			throw std::exception("SequenceTable<K, T>::remove.");
		}
	}
